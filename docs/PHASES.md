    ...
    subphases:
      - id: M11.2
        ...
        acceptance:
          ...
          - id: M11.2-a2
            met: false  # 2026-08-31: the vmhost half is verified green (TestMachineConfigFromSpec, TestLifecycleStateMachine, TestVMAvailableRequiresEntitledHelper, -race); what keeps this row unmet is ONLY the artifact-ensure clause (corrupted-byte reject, retention, --guest-artifacts-dir refusal), which belongs to d3 and has not landed
          ...
          - id: M11.2-a3
            met: false  # 2026-08-31: plan tables + reaper -race verified green on darwin, and the GOOS=linux GOARCH=arm64 CGO_ENABLED=0 build+vet lane verified green by hand; what keeps this row unmet is ONLY the cpio-writer golden, which belongs to d3 (the initramfs producer) and has not landed
