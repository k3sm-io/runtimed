      ...
      - id: M2.7
        ...
        deliverables:
          ...
          - id: M2.7-d2
            ...
            desc: pkg/runtime/exec.go — PortForward dials the pod's lo0 pod IP (darwin-net M2.1 alias; loopback on-node) at the requested port and proxies bytes both directions over the stream, multiplexing connections by connection_id; clean teardown closes all conns + reader goroutines on stream end / ctx cancel. Attach follows a RUNNING container's combined output live (logBuffer gains a bounded follower seam, drop-on-slow so the supervisor log pump never blocks) + delivers the exit code; interactive stdin/tty attach is reported Unimplemented (native pods are posix_spawn'd with stdin NOT retained — documented limitation, kubectl exec is the interactive path) rather than faked. B282 write-back 2026-09-13: the in-memory logBuffer is gone; every container's stdout and stderr are two pipes pumped through pkg/crilog into <PodBox.log_directory>/<container>/<n>.log in the CRI log format (16 KiB P/F chunking, 0600), ContainerStatus.log_path and last_termination_state.log_path name the files, ReopenContainerLog swaps the fd for the node's rotation, GetLogs is Unimplemented (the node reads the file as the kubelet does), and Attach is live-only as upstream CRI attach (the retention-free fanout replaced the bounded replay)
        acceptance:
          - id: M2.7-a1
            met: true
    ...
    subphases:
      - id: M11.2
        ...
        deliverables:
          ...
          - id: M11.2-d3
            done: true  # 2026-08-31, B108+B111 (runtimed#100/#101/#102/#103, k3sm#250) — pkg/guestartifacts pin+ensure+per-boot locator, the reproducible guest-kernel build (dual-anchor PGP first mint, two-run IDENTICAL), the published linux-guest v6.18.48-k3sm.1 release, and the daemon wiring on both node paths. Desc reconciled same day: the shipped acquisition is FETCH-FROM-RELEASE at daemon start (the goreleaser-bundling clause moved to the packaging item), and the dev override is structural (the flag exists only on the dev subcommand tree) rather than a launchd-detection refusal.
            desc: "Guest artifacts (B108 mechanism / B111 producer, delegated): EnsureGuestArtifacts — daemon-start ensure, never lazy-on-first-pod (the M7.1-d1 no-runtime-fetch posture; kernel+initramfs stay out of the bottle for size and are fetched from the k3sm-io/linux-guest release at install/start under verify-then-use; air-gap = pre-seed); sha256 pinned in code under the composite kernelver-configver identity, verify-then-delete-on-mismatch, per-boot locator re-verification (no OS code-signing backstop on a VZ-booted kernel), sha-keyed N-1 retention re-verified on touch; VMArtifactsAvailable condition + the k3sm.io/vm-artifacts label. initramfs = cpio-wrapped k3sm-guest-init (pure-Go newc writer, byte-deterministic: zeroed mtimes/uid/gid, sorted entries — golden-tested), composed by the guest-initramfs CLI and published with the kernel; the pin covers the released pair. The dev artifacts dir is dev-subcommand-only by construction (no such flag on server/agent, flag-set-tested)."
          - id: M11.2-d4
            ...
            desc: "pkg/guestinit + cmd/k3sm-guest-init (B102): pure plan producers (mount plans incl. per-container /etc bind set + musl-safe search list, Rosetta binfmt line — flags POCF, F-rationale recorded, user resolution, sidecar apply (the PLAN step only: the in-guest ownership apply itself landed 2026-09-24 via runtimed#163 after triage runtimed#161 found ownership.jsonl written but never applied), tmpfs-upper size bound) darwin-testable; the PID1 reaper + Stop(grace) state machine GOOS-PORTABLE behind a proc seam with a NAMED -race suite (the milestone's most concurrency-sensitive code); cmd/ is the thin GOOS=linux CGO=0 executor. Boot: mounts → guest-spec.json → per-container overlay (virtiofs lower RO + bounded tmpfs upper, metacopy=on) → sidecar → binfmt → hostname → /etc binds into EVERY container rootfs (chroot shadows guest /etc — kubelet contract) → eth0 DHCP (pure-Go; the agent-Health lease is the SINGLE live-address authority — the 'runtimed reconciles from the attachment' comments in vm.go/guest.go are retired; closes darwin-net M5.2 caveat (a)) → init then main containers (cgroup2 leaf, chroot, workdir, k8s uid/gid precedence, fsGroup supplemental + idmapped volume mounts) → reap → Stop: TERM→grace→KILL→sync→poweroff. Guest link MTU ≤1380 if cross-node is ever claimed. Cross-build + vet lane wired into runtimed's per-CI gate."
          - id: M11.2-d5
          ...
          - id: M11.2-d9
            done: true  # 2026-08-31 — the live-boot spine: CreateVM spawns/supervises/tears down one k3sm-vmhost per vm pod; unit-gated by a8 and proven live by a9's committed, re-runnable smoke on the entitled rig
            desc: "The LIVE-BOOT SPINE (FILED WITH THE DELIVERABLE 2026-08-31 — the d8-gap class: pkg/vmhost's package doc already cited this work as 'a separate deliverable' and the frontmatter carried no row for it, so the one thing that made every other M11.2 deliverable reachable had no ledger entry). pkg/sandbox.CreateVM stops being ErrVMBootNotImplemented: it marshals the pod's VMHostSpec against guestv1+protojson DIRECTLY (never importing pkg/vmhost — that would drag Code-Hex/vz into the daemon and dissolve the entitlement split; the two ends meet at guest/v1, which is versioned for exactly this), creates the pod-contained share roots VZ refuses to attach when absent (found by the live smoke, not by review), spawns ONE k3sm-vmhost via pkg/supervisor's existing primitives (posix_spawn SETSID + signal-mask reset + combined-output capture + the sole KqueueReaper), and waits for readiness. READINESS IS A REAL guest/v1 Health ROUND TRIP, never a socket dial — the helper binds agent.sock BEFORE the machine exists, so a connect would report every pod Running the instant its helper started — and the poll RACES Process.Done() so a helper that dies pre-ready fails in milliseconds carrying its own stderr tail instead of burning the deadline and naming the wrong cause. Deadline 30s, DERIVED: S1 measured 165ms median create->token; the d9 live smoke measured the whole spawn->Health chain at 668-707ms over five runs, so ~40x the observed worst case covers multi-pod contention while staying bounded. Every failure stays FAILURE_REASON_SANDBOX_SETUP at the enum level and names its sub-cause in the message (artifacts-missing / spec-write-failed / helper-not-found / spawn-failed / helper-died-pre-ready + stderr tail / agent-never-ready / canceled) plus the pod's console.log path. ARTIFACTS come through an INJECTED locator seam (VMSpec carries no kernel/initramfs paths); unset ⇒ FAIL CLOSED with ErrGuestArtifactsUnavailable rather than booting an unpinned kernel off a guessed path — the production feeder stays d3/B111. TEARDOWN SHIPS IN THE SAME SLICE, because a boot with no stop is a leak: DeletePod grows a vm branch (its two-phase container teardown iterates a containerProc list a vm pod does not have, so without it a delete silently left a live machine), Runtime.Close grows the carve-out OPPOSITE the host-pod rule (host processes survive a daemon restart BY DESIGN; a vmhost must not — helpers are stopped CONCURRENTLY because serial stops blow launchd's 45s ExitTimeOut at two vm pods), a SEPARATE vm-only orphan store + sweep kills helpers a kill -9'd daemon left holding live machines (ALWAYS KILL, NEVER ADOPT — the exact opposite of podreap's keep-alive policy, which is why it is deliberately not that code path, with the same pgid+immutable-start exact-instance identity discipline), and a post-ready exit watch fails a pod whose helper dies so a hypervisor crash cannot wedge it at Running. ONE GRACE BUDGET across the process boundary: cmd/k3sm-vmhost gains -stop-grace, the daemon computes the same clamp and never escalates on a shorter timer (the two-independently-clocked-timers power cut); the mirrored bounds are pinned by pkg/vmhost.TestStopGraceBoundsAreSingleValued. RestartContainer forks to typed UNIMPLEMENTED at the TOP, before any .proc touch (the d6 contract). CONFINEMENT DECIDED BY MEASUREMENT (m11-plan R22 admits either, as a NAMED choice): the helper runs UNCONFINED. Spawning it under the pod's own Generate() profile was built and run on the entitled rig and refused twice — `ls: <work-dir>/run/vm/<pod>: Operation not permitted` (the profile denies the run tree the agent socket must live in, BY DESIGN so no pod SBPL can ever allow an agent.sock) and, with the socket moved into the re-allowed pod dir to isolate the second leg, `VZErrorDomain Code=1 \"Failed to issue Fuse sandbox extension.\"` (a confined process cannot issue the virtiofs extension VZ needs — exactly the device surface S1(5) disclaimed). Both denials + the residual are recorded verbatim at spawnVMHost, alongside the R19(c) publisher-identity/CDHash residual Available()'s designated-requirement check does not cover. Also closes two adjacent defects the design review found in landed code: vmhost.Lifecycle.Run now force-stops a machine a cancelled Start left half-booted, and a C-LEVEL guard (pkg/sandbox/vmshim_guard_test.go) pins vm_darwin.m's load-bearing entry-point count across the .m/.h pair and scans it for VM-construction selectors — the Go import guard is blind to cgo, and that file already links Virtualization into the daemon."
          - id: M11.2-d10
            done: true  # 2026-08-31 — the guest-spec writer: CreateVM composes and commits guest-spec.json into the k3sm.spec share root before the spawn, so pkg/guestinit finally has a producer
            desc: "The GUEST-SPEC WRITER (FILED WITH THE DELIVERABLE 2026-08-31 — the d9/d8 unfiled-gap class: pkg/guestinit has read guest-spec.json since it was written, d9 shipped the share root EMPTY with a comment deferring the file to 'the guest-provisioning slice', and no ledger row named that slice). pkg/sandbox gains buildGuestSpec — a pure, OS-independent composer from VMSpec + its virtiofs share plan + its GuestNetworkConfig onto guestv1.GuestSpec — and writeGuestSpec, which commits it atomically (temp+rename, mode 0444) into the pod's k3sm.spec share root inside the d9 boot flow, BEFORE the helper is spawned. Three mappings are load-bearing. (1) The DNS configuration crosses from the STRUCTURED triple ONLY: GuestNetworkConfig carries it twice and reading the rendered string would re-parse the host's own output to refill the message the proto shape exists to keep structured, so a config carrying only the rendered string yields NO resolv_conf (asserted, because the bug is invisible in a golden). (2) agent_port and rosetta are MIRRORED from VMAgentVsockPort and VMHostRosettaShareSupported — the constants that already decide them — so the pair of specs one host writes together cannot disagree the way guest.proto forbids (R26 keeps rosetta false; the composer mirrors, it does not decide). (3) The share plan is PER-CONTAINER and guest/v1's mount list is POD-LEVEL, so the mapping is a checked FLATTENING: identical requests collapse, two containers wanting different sources at one guest path is a fail-closed rejection, a claim mounted whole becomes a virtiofs device at its mount path, and a volume inside a pooled share needs a staging mount whose exposure residual is RECORDED on guestShareStageRoot (guestinit re-exposes every pod mount in every container) rather than papered over. Tamper-evidence is the write-then-spawn ORDERING plus the VZ device flag (vmhost.forcedReadOnlyTags forces k3sm.spec read-only); 0444 is defence in depth against a host-side accident, never the enforcement point. VMSpec gains Hostname / FSGroup / Containers ([]VMContainer) on the existing plain-data decoupling seam. PRODUCER GAP, stated rather than hidden: NOTHING STAMPS Containers YET — createVMPod holds the PodBox, the image config and pkg/image.MergeRunSpec, so the container-resolution mapper is its own deliverable — and the composer deliberately does NOT reject an empty container list, because guestinit.Plan is the right refuser (it is PID 1 of a VM that exists to run exactly this pod). Until that mapper lands a vm pod's written spec carries no containers and the guest refuses it with its own reason."
          - id: M11.2-d11
            done: true  # 2026-08-31 — FILED WITH THE DELIVERABLE (the d8/d9/d10 unfiled-gap class): createVMPod stamps VMSpec.Containers through the REAL image-config merge (four quadrants, $(VAR), the runAsNonRoot rule), so the written guest spec finally carries the pod's containers. The vm path now pulls (the merge needs a verified image config) but deliberately does not materialize per-container trees. Fail-closed rows: absolute-path/host-binary image (no host binary can run in a guest), non-numeric image USER with no runAsUser (the guest/v1 user-string gap — carried by the filed additive-carve item), native sidecar (a never-exiting init bit would hang the boot forever). Proven by the boot-contract test: the real CreateVM writes the spec, the real guest-init plans it, and the asserted argv/env/tags are merge-derived values the fixture never handed the composer
            desc: "The vm container mapper: createVMPod resolves each container (init + regular, order preserved) through image.MergeRunSpec and resolveCredential onto sandbox.VMContainer — argv/env/workdir from the merge only (never the host-process containerEnv, whose TMPDIR/DYLD entries are macOS facts), numeric identity or fail closed, the rootfs share tag read off the share plan never restated. Gate: pkg/runtime.TestVMPodBootContractSatisfiesGuestInit + the resolveVMContainers tables."
        acceptance:
          - id: M11.2-a1
            met: true  # 2026-08-31 write-back — both named tests exist and run green under -race; the deliverables they check (d0 platform selection, d1 rootfs builder) landed 2026-08-10/2026-08-30 and this row was never flipped
          ...
          - id: M11.2-a2
            met: true  # 2026-09-01 (M11 validation): the clauses the 2026-08-31 note named as outstanding have LANDED with d3 and were re-run green on an arm64 rig — pkg/guestartifacts TestEnsureGuestArtifacts{,IsIdempotent,CleansPartialDownloads,FetchTimeout,Retention} cover the artifact-ensure clause (corrupted-byte reject, retention, partial-download cleanup). vmhost half re-confirmed: TestMachineConfigFromSpec, TestLifecycleStateMachine, TestVMAvailableRequiresEntitledHelper all ok.
          ...
          - id: M11.2-a3
            met: true  # 2026-09-01 (M11 validation): the cpio-writer golden the 2026-08-31 note named as outstanding has LANDED with d3 — pkg/guestartifacts.TestComposeInitramfs{,IsDeterministic,RoundTrip} ok. Plan tables + reaper re-run green (TestPlanRealizesTheGoldenSpec, TestLinuxMountFlags, TestPID1ReapAndForward), and the GOOS=linux GOARCH=arm64 CGO_ENABLED=0 build+vet lane re-verified on an arm64 host.
            ...
            method: unit
            test: pkg/guestinit.TestPlanRealizesTheGoldenSpec + pkg/guestinit.TestLinuxMountFlags + pkg/guestinit.TestPID1ReapAndForward  # name corrected 2026-08-31: no TestMountPlan was ever written; the plan tables are proven by the golden-spec and mount-flags suites
          - id: M11.2-a4
            met: true  # 2026-08-31 write-back — all three named tests exist and run green under -race (d2 vmhost + d5/d6 exec/stats landed with the vmhost PR); the row was never flipped
          ...
          - id: M11.2-a5
            met: true  # 2026-09-01 (M11 validation): verified on the shipped module view rather than by reading the import list. GOWORK=off `go mod why -m github.com/Code-Hex/vz/v3` reports "main module does not need module github.com/Code-Hex/vz/v3"; `go list -deps ./cmd/k3sm` carries neither vz nor runtimed/pkg/vmhost; `nm` on the built k3sm binary finds 0 vz symbols. The workspace-on reading traces vz through runtimed/pkg/vmhost, which is the separate entitled helper and is expected.
            check: "the k3sm product binary's PACKAGE and LINK graphs exclude github.com/Code-Hex/vz: go list -deps ./cmd/k3sm carries neither vz nor runtimed/pkg/vmhost, go mod why -m github.com/Code-Hex/vz/v3 reports does-not-need, and the binary carries no vz Go symbols — NOT 'links no Virtualization framework', which is unsatisfiable BY DESIGN: pkg/sandbox/vm_darwin.go declares `#cgo LDFLAGS: -framework Virtualization` for the Rosetta availability probe shim, so the product binary always links the framework while importing zero vz Go packages (clause corrected 2026-08-31); CORRECTION 2026-08-31 (same day): the runtimed-side guard DOES exist and is better than a ci stage — pkg/vmhost/importguard_test.go::TestVZIsNotReachableFromTheDaemon walks go list -deps over all five named packages plus guestagent, volume and cmd/k3sm-runtimed as an explicit list, carries a converse leg so removing vz entirely cannot fake a pass, and fires under plain go test in the PR that adds the bad import; the k3sm-repo facts also hold, verified by hand under GOWORK=off (go list -deps ./cmd/k3sm carries neither vz nor vmhost; go mod why -m reports does-not-need). What keeps this row unmet is ONLY the k3sm-side ci canary (package-graph + no-vz-symbols check in that repo's gate), which is implemented nowhere; a runtimed-side go list -deps guard over pkg/{runtime,sandbox,image,mount,supervisor} fires in the PR that adds a bad import. RECORDED EXCEPTION: k3sm/go.sum carries a /go.mod-only hash line for Code-Hex/vz (module-graph pruning loads runtimed's go.mod), asserted to carry no bare h1: zip line. CORRECTED 2026-08-30: the prior text said module graph, but go list -deps is a PACKAGE-graph tool and the literal module graph cannot be kept clean while vmhost lives in the runtimed module (a deliberate siting decision — one version pin with apis/guest/v1, not two). internal/spicanary byte-unchanged; live VM boot / Rosetta / virtiofs-attach legs are the M11.5 lab gate (hack/lab/m11.sh), never auto-greened"
            method: build
          - id: M11.2-a6
          ...
          - id: M11.2-a8
            met: true  # 2026-08-31 — FILED WITH THE DELIVERABLE (a1-a7 name no boot-spine test). All named tests green under -race; the C-guard's non-vacuity was proven by MUTATION of the real vm_darwin.m (a planted [[VZVirtualMachineConfiguration alloc] init] and a smuggled fifth entry point each go red, then restored), and the share-root row was authored FROM a live-boot failure rather than from review.
            check: "the vm boot spine with a fake spawner/reaper/agent and no VM: the written vmhost.spec.json decodes as a guestv1.VMHostSpec with unknown fields REJECTED and carries the inverted fail-closed read-only flags, the appended-by-the-helper k3sm.spec share root, and the clamped -stop-grace argv; an unset/failing/half-empty artifact locator fails CLOSED before any spawn; a helper that dies pre-ready is reported as helper-died-pre-ready WITH its stderr tail and in well under the deadline (never as agent-never-ready); a helper that never becomes ready is signalled and leaves no handle and no reap record; StopVM never waits less than the helper's own clamped budget and is idempotent; StopAllVMs is provably CONCURRENT (wall time against a deliberately slow stop); the orphan decision kills a matching leader, drops a recycled pgid unsignaled, keeps an uninspectable group, and refuses pgid<=1; the sweep's recursive delete is bounded to <root>/run separator-aware; pod-contained share roots are created and out-of-pod (claim) roots are NOT fabricated; the runtime assembles a Running vm pod with no host memory sampler, DeletePod stops the helper with the pod's own grace, Close stops helpers while leaving host pods running, the startup sweep runs exactly once, a post-ready helper exit fails the pod as VMHostExited, and RestartContainer returns Unimplemented; the vmhost lifecycle force-stops a cancelled Start; the mirrored stop-grace bounds agree across the import boundary; the cgo shim's entry-point count and construction-selector scans hold. All -race"
            method: unit
            test: pkg/sandbox.TestCreateVMWritesTheMachineDescription + pkg/sandbox.TestCreateVMSurfacesAPreReadyHelperDeath + pkg/sandbox.TestStopAllVMsFansOut + pkg/sandbox.TestVMReapDecision + pkg/runtime.TestCreateVMPodAssemblesARunningPod + pkg/runtime.TestDeleteVMPodStopsTheHelper + pkg/runtime.TestCloseStopsVMHelpersButNotHostPods + pkg/runtime.TestVMHelperExitFailsTheRunningPod + pkg/vmhost.TestStopGraceBoundsAreSingleValued + pkg/sandbox.TestVMShimHasNoVMConstruction
          - id: M11.2-a9
            met: true  # 2026-08-31 — the live smoke is a RUNNABLE ARTIFACT, not a report: all three legs green on the entitled rig (leg 1 create->Health->Running 1.02s; leg 2 delete->helper exit + socket gone via the ESCALATION arm; leg 3 kill -9 of a real daemon subprocess -> helper survives (SETSID) -> the next Runtime's sweep kills it and clears the run dir), and green again under -race. It is NOT auto-greened: it skips unless K3SM_CAP_VZ=1 AND the built helper probes Available().
            check: "the live vm-boot smoke, on a declared VZ lab: a REAL k3sm-vmhost built and ad-hoc-signed from this tree with the shipped vmhost.entitlements, a REAL Virtualization machine booted from a lab-declared kernel plus an initramfs COMPOSED AT TEST TIME from testdata/vmsmokeguest (never a committed binary; every artifact's sha256 logged), driven by a REAL in-process runtime.Runtime. Leg 1: CreatePod reaches RUNNING through a genuine guest/v1 Health handshake, then a SECOND Health round trip through the daemon's own GuestDialer proves the socket the helper bound is the socket Exec/GetLogs reaches. Leg 2: DeletePod drives the ESCALATION arm (the lab guest ignores SIGTERM by design), the helper pid provably exits and agent.sock is gone. Leg 3: a re-executed daemon SUBPROCESS boots the pod and is SIGKILLed, its helper is asserted to SURVIVE (else the leg is vacuous), and a fresh Runtime's ReapOrphanedPods kills it and clears the run dir. t.Cleanup SIGKILLs the helper process group on every exit path. Gated by k3smtest.SkipUnless(VZ) BEFORE anything is built, plus an Available() PROBE that FATALs rather than skips on a rig that declared VZ but carries an unusable helper. Deliberately NOT behind the integration build tag — a lab-only gate that stops COMPILING is a gate nobody notices is broken (the TestIntegrationMetalMatmulUnderProfile precedent)."
            method: integration
            test: pkg/runtime.TestIntegrationVMBootSmokeLifecycle + pkg/runtime.TestIntegrationVMBootSmokeOrphanSweep
          - id: M11.2-a10
            met: true  # 2026-08-31 — FILED WITH THE DELIVERABLE (a1-a9 name no guest-spec-producer test). All named tests green under -race with the full suite; the live smoke's new post-boot assertion rides the existing env-gated leg 1 and skips with it. Non-vacuity: the structured-resolver row FAILS if the composer is made to read GuestNetworkConfig.ResolvConf, and the ordering row FAILS if the write is moved after the spawn — each was checked by mutation, then restored.
            check: "the guest boot spec, both ends: a fully-populated VMSpec composes to a committed guest-spec.json golden (compared DECODED with unknown fields REJECTED — protojson varies its whitespace on purpose, so a byte golden would be flaky about nothing); the composed spec is fed to the REAL pkg/guestinit.Plan and every virtiofs source that plan would mount is a tag the share plan declared, each container's rootfs lower is its own declared tag, and init containers order first; a GuestNetworkConfig carrying ONLY the rendered resolv.conf string yields an EMPTY resolv_conf (no string re-parsing path exists) and an absent guest network still plans; a bind or rootfs tag the share plan does not carry, a duplicated share tag, two containers claiming one guest path, and a non-integer emptyDir size limit each fail CLOSED before any spawn; the written file lands in the k3sm.spec share root mode 0444 and already exists at the first post-spawn observation point; agent_port and rosetta equal the VMHostSpec's. Live rig: the spec on the real share root decodes strictly post-boot (rides the env-gated smoke leg 1)."
            method: unit
            test: pkg/sandbox.TestGuestSpecGolden + pkg/sandbox.TestGuestSpecRoundTripsThroughGuestInit + pkg/sandbox.TestBuildGuestSpecReadsOnlyTheStructuredResolver + pkg/sandbox.TestBuildGuestSpecFailsClosed + pkg/sandbox.TestBuildGuestSpecMapsTheSharePlan + pkg/sandbox.TestCreateVMWritesTheGuestSpec + pkg/sandbox.TestCreateVMRefusesAnUncomposableSpec + pkg/runtime.TestIntegrationVMBootSmokeLifecycle
...
- ✅ `M2.7-d2` `PortForward` dials the pod's **lo0 pod IP** (darwin-net M2.1 alias; loopback on-node) at
  ...
  `Attach` follows a **running** container's live output (live-only since B282 on 2026-09-13; the
  retention moved to the on-disk CRI log files written by `pkg/crilog`) and delivers the exit code; interactive
...
**Deliverables** — see the frontmatter `M11.2-d0…d9` for the binding detail: d0 platform selection
...
OOMKilled-from-events-only, restart = whole-pod recreate); d8 the guest-network consumer seam;
**d9 the live-boot spine** — `CreateVM` spawns, supervises and tears down one `k3sm-vmhost` per vm
pod, with readiness as a real `guest/v1` `Health` round trip raced against the helper's exit, one
grace budget shared across the process boundary (`-stop-grace`), a vm-only always-kill orphan sweep,
and the helper running **unconfined** as a measured choice (both Seatbelt denials recorded).
...
**Acceptance (exit gate)** — frontmatter `M11.2-a1…a9`; the *multi-pod / Rosetta / virtiofs-idmap*
legs stay the M11.5 lab gate (`hack/lab/m11.sh`), never auto-greened. The single-pod boot leg is no
longer only lab-gated: d9's `a8` proves the spine unit-side against fakes, and it was additionally
run live on an entitled rig. That live run is now a COMMITTED, RE-RUNNABLE artifact rather than a
