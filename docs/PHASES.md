    ...
    subphases:
      - id: M11.2
        ...
        deliverables:
          ...
          - id: M11.2-d9
            done: true  # 2026-08-31 — the live-boot spine: CreateVM spawns/supervises/tears down one k3sm-vmhost per vm pod; unit-gated by a8 and proven live on the entitled rig (create->Health 666ms; delete->socket gone; kill -9 daemon -> orphan swept)
...
**Acceptance (exit gate)** — frontmatter `M11.2-a1…a8`; the *multi-pod / Rosetta / virtiofs-idmap*
...
run live on an entitled rig (`create → Health` 666 ms, `delete → agent.sock` gone, `kill -9` of the
daemon → the next start's sweep killed the orphaned helper).
