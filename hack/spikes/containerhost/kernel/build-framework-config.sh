...
# SPIKE: extract inside the container filesystem, not the virtiofs host mount — the host
# mount raised "Directory renamed before its status could be extracted" from GNU tar on
# 2026-09-09 (a Docker Desktop file-sharing race, not a tarball defect; the 2026-08-31
# production run extracted the same tarball into the same kind of dir fine).
mkdir -p /kbuild
tar -xf "/src/$KERNEL_TARBALL" -C /kbuild
cd "/kbuild/linux-$KERNEL_VERSION"
...
# SPIKE: the framework's config decides modules; k3sm's no-loader rule is guest-init's, not this kernel's.
...
normalize_config() {
  note "normalizing the framework's config-arm64 to its olddefconfig fixed point for linux-$KERNEL_VERSION"
  local work="$CACHE_DIR/normalize"; rm -rf "$work"; mkdir -p "$work"   # under /Users: Docker Desktop shares it
  fetch_pinned
  cp "$CONFIG_FILE" "$work/kernel.config"
  docker run --rm -i --platform linux/arm64 \
    -v "$CACHE_DIR/$KERNEL_TARBALL:/src.tar.xz:ro" -v "$work:/work" \
    "$TOOLCHAIN_IMAGE" bash -eu <<EOS
apt-get -qq update >/dev/null && apt-get -qq install -y $BUILD_DEPS >/dev/null
mkdir -p /kbuild && tar -xf /src.tar.xz -C /kbuild --strip-components=1 && cd /kbuild
cp /work/kernel.config .config
make ARCH=arm64 olddefconfig >/dev/null
cp .config /work/kernel.config
EOS
  cp "$work/kernel.config" "$CONFIG_FILE"
  echo "  wrote $CONFIG_FILE (fixed point; sha256 $(shasum -a 256 "$CONFIG_FILE" | awk '{print $1}'))"
}
...
main() {
  ...
  case "${1-}" in
    ...
    --normalize-config) mode="normalize" ;;
    -h|--help)      usage; exit 0 ;;
    *)              usage >&2; die "unknown argument: $1" ;;
  esac

  if [ "$mode" = "normalize" ]; then
    preflight; normalize_config; exit 0
  fi
  if [ "$mode" = "regen" ]; then
