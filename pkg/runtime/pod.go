...
func (r *Runtime) resolveBinary(ctx context.Context, box *runtimev1.PodBox, rootfs string, c *runtimev1.Container) (path string, argv []string, hostBinary bool, err error) {
	...
	if _, err := r.puller.Pull(ctx, c.GetImage(), cred, nativePullPolicy()); err != nil {
...
// nativePullPolicy is the image-platform policy for the host-process (Mach-O)
// spine — the ONLY spine that pulls today (B99).
//
// It is native, not "whatever the pod asked for", and that is correct rather
